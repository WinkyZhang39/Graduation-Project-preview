...
from utils.WPMD import WPMD_Block
...
class TinyViTBlock(nn.Module):
    ...
    def forward(self, x):
        # print("x shape:", x.shape)#x shape: torch.Size([4, 4096, 128])
        H, W = self.input_resolution
        # print("H, W:", H, W)#H, W: 128 128
        B, L, C = x.shape
        # print("B, L, C:", B, L, C)#B, L, C: 4 4096 128
        assert L == H * W, "input feature has wrong size" #error
        res_x = x
...
class TinyViT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000,
        ...
        self.wpmd1=WPMD_Block(in_channels=3, out_channels=64, downsample=True)
        ...
        # self.wpmd2=WPMD_Block(in_channels=3, out_channels=256, downsample=True)
    ...
    def set_layer_lr_decay(self, layer_lr_decay):
        ...
        self.wpmd1.apply(lambda x: _set_lr_scale(x, lr_scales[0]))
        # self.wpmd2.apply(lambda x: _set_lr_scale(x, lr_scales[0]))
    ...
    def forward_features(self, x):
        ...
        x_image_after_wpmd1 = self.wpmd1(x_image)
        # print("After first WPMD:", x_image_after_wpmd1.shape)
        
        # 确保维度匹配，将x_image_after_wpmd1调整为与x相同的形状
        if x_image_after_wpmd1.shape[2:] != x.shape[2:]:
            # 使用自适应池化调整空间维度
            x_image_after_wpmd1 = F.adaptive_avg_pool2d(x_image_after_wpmd1, x.shape[2:])
        
        # 确保通道数匹配
        if x_image_after_wpmd1.shape[1] != x.shape[1]:
            # 使用1x1卷积调整通道数
            channel_adjust = nn.Conv2d(x_image_after_wpmd1.shape[1], x.shape[1], kernel_size=1).to(x.device)
            x_image_after_wpmd1 = channel_adjust(x_image_after_wpmd1)
        
        x = x + x_image_after_wpmd1  # 融合特征
    ...
    def forward(self, x):
        x = self.forward_features(x)#error
        #x = self.norm_head(x)
        #x = self.head(x)
        return x
